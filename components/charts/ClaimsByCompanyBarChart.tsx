...
interface ClaimsByCompanyBarChartProps {
    data: Claim[];
}

export const ClaimsByCompanyBarChart: React.FC<ClaimsByCompanyBarChartProps> = ({ data }) => {
    const chartData = useMemo(() => {
        const counts = data.reduce<Record<string, number>>((acc, claim) => {
            acc[claim.empresa] = (acc[claim.empresa] || 0) + 1;
            return acc;
        }, {});
                ...
                <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                    <defs>
                        <linearGradient id="companyGradient" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor={CHART_COLORS.blue[0]} stopOpacity={0.8}/>
                            <stop offset="95%" stopColor={CHART_COLORS.blue[1]} stopOpacity={1}/>
                        </linearGradient>
                    </defs>
                    ...
                    />
                    <Bar dataKey="reclamos" fill="url(#companyGradient)" radius={[4, 4, 0, 0]} />
                </BarChart>
            </ResponsiveContainer>
        </div>
    );
